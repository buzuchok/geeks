                                  ...
                                  <div v-if="counter > 0" class="col-xl-1">
                                    <div class="submit-field">
                                      ...
                                      <a
                                        ...
                                        href="javascript:;"
                                        @click="
                                          deleteProductDOM(counter, prsub.prid)
                                        "
                                        ><i
                                          class="fa fa-trash"
                                          aria-hidden="true"
                              ...
                              <div class="col-xl-6">
                                <div class="submit-field">
                                  ...
                                  <div class="uploadButton margin-top-10">
                                    ...
                                    <ul id="uploadpic_list">
                                      <li
                                        v-for="piclist in project.picture"
                                        :key="piclist"
                                        class="dbimg"
                                        v-bind:dataid="piclist"
                                      >
                                        <img
                                          :src="`${image_path}${piclist}`"
                                        /><i
                                          @click="onPictureRemove(piclist)"
                              ...
                              <div class="col-xl-6">
                                <div class="submit-field">
                                  ...
                                  <div class="uploadButton margin-top-10">
                                    ...
                                    <ul id="uploaddia_list">
                                      <li
                                        v-for="dialist in project.diagram"
                                        :key="dialist"
                                        v-bind:dataid="dialist"
                                        class="dbimg"
                                      >
                                        <img
                                          :src="`${image_path}${dialist}`"
                                        /><i
                                          @click="onDiagramRemove(dialist)"
                                          class="fa fa-times-circle"
                                        ></i>
                                      </li>
                                    </ul>
...
export default {
  ...
  methods: {
    async listCategory() {
      const categoriesList = await this.callApi(
        "post",
        window.api_path + "project/category/get"
      );
      if (categoriesList.status == 200) {
        if (categoriesList.data.code == 1) {
          var category = [];

          Object.keys(categoriesList.data.data).forEach(function(key) {
            category.push({
              value: categoriesList.data.data[key].id,
              text: categoriesList.data.data[key].title
            });
          });
    ...
    async getProjectDetail(slug) {
      ...
      if (details.status == 200) {
        ...
        if (details.data.code == 1) {
          this.project.name = details.data.data.name;
          this.project.slug = details.data.data.slug;
          this.project.category = details.data.data.category;
          this.project.type = details.data.data.type;
          this.project.desc = details.data.data.desc;
          this.project.prbudget = details.data.data.budget;

          this.project.picture = details.data.data.picture;
          this.project.diagram = details.data.data.diagram;

          this.project.tags = details.data.data.tags;
          this.project.status = details.data.data.status;

          var thislist = this;
          if (details.data.data.product.length > 0) {
            Object.keys(details.data.data.product).forEach(function(key) {
              thislist.product.push({
                prid: details.data.data.product[key].id,
                prname: details.data.data.product[key].name,
                prlink: details.data.data.product[key].link,
                prprice: details.data.data.product[key].price
              });
            });
        ...
        this.$toaster.error("Project details not found, please try again");
        this.isLoading = false;
        this.$router.push({ name: "projectlist" });
      }
    },
    onPictureRemove(filename) {
      this.project.removepicture.push(filename);
      $("#uploadpic_list li.dbimg[dataid='" + filename + "']").remove();
    },
    onPictureChange(e) {
    ...
    onDiagramRemove(filename) {
      this.project.removediagram.push(filename);
      $("#uploadpic_list li.dbimg[dataid='" + filename + "']").remove();
    },
    onDiagramChange(e) {
      let files = e.target.files || e.dataTransfer.files;
    ...
    createDiagramImage(file) {
      ...
      setTimeout(() => {
        $("#uploaddia_list li.selectedimg").remove();
    ...
    async editProject() {
      if (this.project.name == "") {
      ...
      } else if (this.project.status.length == 0) {
        ...
        var removeproduct_json = "";
        if (this.project.removeproduct.length > 0) {
          removeproduct_json = JSON.stringify(this.project.removeproduct);
        }

        var removepicture_json = "";
        if (this.project.removepicture.length > 0) {
          removepicture_json = JSON.stringify(this.project.removepicture);
        }

        var removediagram_json = "";
        if (this.project.removediagram.length > 0) {
          removediagram_json = JSON.stringify(this.project.removediagram);
        }

        const add = await this.callApi(
          "post",
          window.api_path + "project/editproject",
          {
            ...
            removeproduct: removeproduct_json,
            removepicture: removepicture_json,
            removediagram: removediagram_json
          }
        );
    ...
    deleteProductDOM(counter, id) {
      this.product.splice(counter, 1);

      if (id != "") {
        this.project.removeproduct.push(id);
      }
    }
  },
  mounted() {
    ...
    // }, 1000);
    var vm = this;
    $("body").on("click", "#uploadpic_list li.selectedimg i", function() {
      var imgsrc = $(this)
        .parent()
        .find("img")
        .attr("src");
    ...
    $("body").on("click", "#uploaddia_list li.selectedimg i", function() {
      var imgsrc = $(this)
        .parent()
        .find("img")
        .attr("src");
